...
class MessageWrapperRepositoryISpec
  ...
  private val messageWrapper = MessageWrapper("id", "<Body/>", Received, ReferenceDataExport)
  ...
  ".insertMessageWrapper" - {
    ...
    "must insert a message wrapper successfully" in {
      ...
      val insertResult     = messageRepository.insertMessageWrapper(messageWrapper.uid, messageWrapper.payload, messageWrapper.status, ReferenceDataExport).futureValue
  ...
  ".findByUid" - {
    ...
    "must return none if uid not found" in {
      ...
      val insertResult = messageRepository.insertMessageWrapper("1234", messageWrapper.payload, messageWrapper.status, ReferenceDataExport).futureValue
      val fetchedRecord = messageRepository.findByUid(messageWrapper.uid).futureValue
  ...
  ".updateStatus" - {
    ...
    "must be able to update the status of an existing message wrapper" in {
      ...
      fetchedBeforeUpdateRecord.value.messageType mustEqual ReferenceDataExport
